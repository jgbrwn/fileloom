...
The new editor source is under `web/editor`; rebuild its checked-in static bundle with `make editor-build`. The first time you run the browser suite, install its Chromium with `npx --prefix web/editor playwright install chromium`. Run the initial desktop/tablet/Android-sized smoke matrix with `FILELOOM_E2E_EMAIL=you@example.com make editor-test` against a running CMS. The longer-term editor migration and comments architecture are documented in [docs/editor-migration.md](docs/editor-migration.md) and [docs/comments-plan.md](docs/comments-plan.md). The Deckflow migration is experimental: selection-aware block insertion falls back to the body end when needed, the media panel supports image selection/upload and drag/drop, the Details sheet handles supported metadata/status/revision actions, code blocks use the legacy semantic language/highlighting contract, theme layouts/tokens remain in the CMS workflow, and Preview renders the current page through the active theme template/layout without writing source or generated output.
