...
The new editor source is under `web/editor`; rebuild its checked-in static bundle with `make editor-build`. The longer-term editor migration and comments architecture are documented in [docs/editor-migration.md](docs/editor-migration.md) and [docs/comments-plan.md](docs/comments-plan.md). The Deckflow migration is experimental: selection-aware block insertion falls back to the body end when needed, the media panel supports image selection/upload and drag/drop, theme layouts/tokens remain in the CMS workflow, and preview uses the public page for published content or the synthetic editor canvas for unpublished content.
